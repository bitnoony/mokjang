<script>
	...
	async function init() {
		const { mokwonList: list } = await getUserListWithMokwon(
			mokjaId,
			mokjang.idx,
		);
		mokwonList = list;
	...
	<div class="list-wrap common-scroll">
		<div class="list-group">
			{#each mokwonList as mokwon}
				<li
					class="list-group-item list-group-item-action list-item mokwon-list-item"
					aria-current="true"
				>
					<input
						class="form-check-input me-3 c-pointer"
						type="checkbox"
						bind:checked={mokwon.isMokwon}
						disabled={mokwon.type === "목자"}
						on:click={e => {
							changeMokwonStatus(e, mokwon);
						}}
					/>
					<div class="d-flex align-items-center justify-content-between w-100">
						<div class="d-flex align-items-center">
							<img
								src={mokwon?.profile_image ?? userImg}
								alt="twbs"
								width="32"
								height="32"
							...
							<span
								class="ms-1 align-bottom badge text-bg-{mokwon.type === '목자'
									? 'danger'
									: 'success'}"
							>
								{mokwon.type}
