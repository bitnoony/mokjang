				...
				<div data-idx="{mokjang.idx}" class="p-2 col-lg-3 col-md-6 col-sm-12">
					<div class="card">
						<div class="card-body">
							...
							<h6 class="card-subtitle mb-2 text-body-secondary">{mokjang.area}</h6>
							<h6 class="card-subtitle mb-2 text-body-secondary">{mokjang.tag ?? ""}</h6>
						</div>
					</div>
				</div>
			{/each}
	...
	<div class="modal-center d-none" bind:this={modal}>
		<div class="modal-content rounded-4 shadow bg-white">
			...
			<div class="modal-body p-5 pt-0">
				...
				<div class="form-floating mb-3">
					<input type="text" class="form-control rounded-3" id="floatingArea" bind:value="{modalArea}">
					<label for="floatingArea">지역</label>
				</div>
				<div class="form-floating mb-3">
					<input type="text" class="form-control rounded-3" id="floatingTag" bind:value="{modalTag}">
					<label for="floatingTag">태그</label>
				</div>
				<div class="mb-3 d-flex justify-content-center">
					<div class="form-check form-switch">
						<label class="form-check-label" for="currentMokjangToggle">현재 목장 여부</label>
						<input class="form-check-input" type="checkbox" role="switch" id="currentMokjangToggle" bind:this={modalCurrentUse}>
					</div>
				</div>
				<button class="w-100 mb-2 btn btn-lg rounded-3 btn-primary" on:click|once={modalSubmit}>확인</button>
				{#if mokjangModalType !== "추가"}
				<hr class="my-4">
				<div class="d-flex justify-content-end">
					<button class="mb-2 btn btn-sm rounded-3 btn-danger" on:click={e => deleteMokjang(e, modalIdx)}>
